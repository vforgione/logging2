...
_name_group = '\w+'
_message_group = '[^\n]+'
BASIC_OUTPUT_REGEX = re.compile(f'^{_timestamp_group} {_level_group} {_name_group}: {_message_group}')
...
class TestLogger:
    ...
    def test_additional_context_static(self):
        ...
        logger = Logger(name='ad-static', template=template, additional_context={'person': 'vince'})
    ...
    def test_additional_context_function(self):
        ...
        logger = Logger(name='ad-func', template=template, additional_context={'uuid': uuid4})

        with CaptureOutput() as co:
            logger.info('hello')
        output = co.get_text()

        assert re.match('[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12} hello', output)
